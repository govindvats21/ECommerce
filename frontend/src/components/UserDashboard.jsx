  ...
  useEffect(() => {
    if (allItems && allItems.length > 0) {
      ...
      allItems.forEach((item) => {
        ...
        const isEcommerce = ecommerceList.some(
          (e) => e.toLowerCase() === cat?.toLowerCase(),
        );
        ...
        <div className="relative w-full overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-transparent to-transparent flex items-center px-12">
            <div className="text-white space-y-4 hidden md:block">
              <span className="bg-blue-600 px-4 py-1 rounded-full text-xs font-black uppercase tracking-[0.3em]">
                New Season
              </span>
              ...
              <p className="text-gray-300 font-medium">
                Get up to 40% off on latest electronics & fashion.
              </p>
            </div>
          </div>
        </div>
              ...
              <div key={index} className="flex flex-col gap-6">
                <div className="flex items-end justify-between px-2 border-b border-gray-100 pb-4">
                  ...
                  <button
                    ...
                    className="group flex items-center gap-2 text-[10px] font-black text-blue-600 uppercase tracking-widest hover:text-black transition-all"
                  >
                    Explore All{" "}
                ...
                <div className="flex overflow-x-auto pb-8 pt-2 no-scrollbar snap-x scroll-smooth">
                  {ecommerceItems[section].map((item) => (
                    <div
                      key={item._id}
                      className="flex-shrink-0 w-[220px] md:w-[230px] snap-start"
                    >
                      <ItemCard data={item} />
                    </div>
                  ))}
