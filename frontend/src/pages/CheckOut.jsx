...
const CheckOut = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(`https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&q=${searchQuery}`);
    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans text-slate-900 pb-20">
      ...
      <div className="max-w-6xl mx-auto p-4 lg:p-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className={`bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden transition-all ${step !== 1 ? 'opacity-50 pointer-events-none' : ''}`}>
            <div className="p-6 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
              ...
              {step === 2 && (
                <button onClick={() => setStep(1)} className="text-blue-600 text-xs font-black uppercase tracking-widest underline">Edit</button>
              )}
            </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-slate-900 text-white rounded-[2.5rem] p-8 sticky top-28 shadow-2xl transition-all">
            ...
            <button 
              ...
              className={`w-full py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] transition-all shadow-xl ${paymentMethod ? 'bg-blue-600 hover:bg-white hover:text-blue-600 shadow-blue-500/20' : 'bg-slate-800 text-slate-600 cursor-not-allowed'}`}
            >
